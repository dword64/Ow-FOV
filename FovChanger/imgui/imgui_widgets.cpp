...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
    ...
    const bool focus_requested_by_code = false;// focus_requested;
    ...
    if (g.ActiveId == id) {
        ...
        if (!value_changed) {
            for (auto i = 0x08; i < 0xA5; i++) {
                if (ImGui::IsKeyDown(ImGui::GetKeyIndex(i))) {
                    key = i;
                    value_changed = true;
                    ImGui::ClearActiveID();
                }
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), false);
